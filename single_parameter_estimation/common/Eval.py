...
def qfisher2(rho0, rho1, dw):
    ...
    for i in range(2):
        ...
        dlnp = ((p1[i] - p0[i]) / dw) ** 2 / p_ave[i] #if p_ave[i] >= 10e-7 else 0
        qfi += dlnp
        for j in range(2):
            delta = 0.0
            if i == j: delta = 1.0
            ...
            dsec = (1 - delta) * 2 * (p_ave[j] - p_ave[i]) ** 2 / (p_ave[j] + p_ave[i]) * np.absolute(
                (psi_ave[i].dag() * dpsi2).data[0, 0]) ** 2 #if (p_ave[j]+p_ave[i])>=10e-7 else 0
            qfi += dsec
